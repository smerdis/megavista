...
2017-Apr-05 AM modified it, based on motionparams_SP.py
""" 
...
if __name__ == "__main__": 
    ...
    if len(sys.argv) > 2: # if we were told explicitly which files to use
        motion_list = sys.argv[2:] # each file is an argument
    else:
    ...
    print t1[:10]
    print T1_differences[:10]
    print t2[:10]
    print T2_differences[:10]
    print T3_differences[:10]
    ...
    for folder in dir_list:
        if folder.startswith('epi'):
            dicom_files = os.listdir(os.path.join(dicom_dir, folder))
