...
2017-Apr-05 AM edited it, modified from motioncorrect_SP.py
""" 
...
if __name__ == "__main__":
    ...
    dicom_dir = os.path.join(sess_dir,(sess_name+'_dicom'))
    nifti_dir = os.path.join(sess_dir,(sess_name+'_nifti'))

    nifti_list = np.array(os.listdir(nifti_dir)) # All niftis in this dir
    print(nifti_list)
    if len(sys.argv) > 2: # if we were told explicitly which files to use
        epi_list = sys.argv[2:] # each file is an argument
    else:
        epi_list = []
    ...
    print "Will motion correct the following files: ", ', '.join(epi_list)
    ...
    print('Creating reference volume from middle epi volume ', mid_epi)
    epi_mid_dir = mid_epi.split('.')[0]
    os.chdir(os.path.join(dicom_dir, epi_mid_dir))
    print os.path.realpath(os.path.curdir)

    # Find middle dicom file
    mid_epi_noext = mid_epi.split('.')[0]
    mid_dicom_dir = os.path.join(dicom_dir,mid_epi_noext)  # take off .nii.gz file extension TODO unhardcode all this
    print(epi_mid_dir, mid_dicom_dir)
